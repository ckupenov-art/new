// Toilet Paper Pack Generator - main script with debug console

let scene, camera, renderer, controls;
let packGroup;
let targetCenter; // will be created once THREE is available
...
// DOM references will be filled on DOMContentLoaded
let inputs = {};
let totalRollsEl, packDimensionsEl, footprintEl;
let generateBtn, exportPngBtn;
let packRotX, packRotY, packRotXVal, packRotYVal;
let camAngleX, camAngleY, camDistance, camPanX, camPanY;
let camAngleXVal, camAngleYVal, camDistanceVal, camPanXVal, camPanYVal;
let clearDebugBtn;
...
function updateResults(rollDiameter, rollHeight, rows, rollsPerRow, layers) {
  ...
  logDebug("Updated results", { totalRolls, packWidth, packDepth, packHeight });
...
// 3D setup
function init3D() {
  logDebug("init3D called");

  if (typeof THREE === "undefined") {
    logDebug("ERROR: THREE is not defined. Script order or CDN load failed.");
    return;
  }
  ...
  if (!container) {
    logDebug("ERROR: canvas-container not found in DOM");
    return;
  }
  ...
  if (!height || height < 50) {
    height = 400;
    logDebug("canvas-container height was 0, using fallback 400px");
  }
  ...
  logDebug("3D scene initialized", { width, height });
}
...
function onWindowResize() {
  ...
  logDebug("Window resized", { width, height });
}
...
function buildPack() {
  ...
  if (!packGroup) {
    logDebug("ERROR: packGroup not initialized yet.");
    return;
  }
...
function applyPackRotation() {
  ...
  logDebug("Pack rotation applied", { rotXDeg, rotYDeg });
}
...
function applyCameraControls() {
  ...
  logDebug("Camera updated", { angleXDeg, angleYDeg, distance, panX, panY });
}

// PNG export
function exportPNG() {
  if (!renderer) {
    logDebug("ERROR: renderer not initialized, cannot export PNG");
    return;
  }
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  packDimensionsEl = document.getElementById("packDimensions");
  footprintEl = document.getElementById("footprint");
  generateBtn = document.getElementById("generateBtn");
  exportPngBtn = document.getElementById("exportPngBtn");

  packRotX = document.getElementById("packRotX");
  packRotY = document.getElementById("packRotY");
  packRotXVal = document.getElementById("packRotXVal");
  packRotYVal = document.getElementById("packRotYVal");

  camAngleX = document.getElementById("camAngleX");
  camAngleY = document.getElementById("camAngleY");
  camDistance = document.getElementById("camDistance");
  camPanX = document.getElementById("camPanX");
  camPanY = document.getElementById("camPanY");

  camAngleXVal = document.getElementById("camAngleXVal");
  camAngleYVal = document.getElementById("camAngleYVal");
  camDistanceVal = document.getElementById("camDistanceVal");
  camPanXVal = document.getElementById("camPanXVal");
  camPanYVal = document.getElementById("camPanYVal");

  clearDebugBtn = document.getElementById("clearDebugBtn");

  logDebug("DOMContentLoaded");
  logDebug("THREE present?", { hasTHREE: typeof THREE !== "undefined" });

  if (typeof THREE === "undefined") {
    logDebug("ERROR: THREE is undefined. Check that the CDN scripts are loading correctly.");
    showError("3D engine could not load (THREE.js missing). Check your internet connection.");
    return;
  }
